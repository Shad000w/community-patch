...
// if set, instead of going back, creature will return
// to first waypoint
const int NW_WALK_FLAG_LOOPING                     = 0x00000010;
...
object GetNextWalkWayPoint(object oCreature=OBJECT_SELF)
{
    ...
    if (nCurWay == -1) {
        ...
        if (bGoingBackwards) {
            ...
            nCurWay++;
            if (nCurWay > nPoints) {
                if(GetWalkCondition(NW_WALK_FLAG_LOOPING, oCreature)) {
                    nCurWay = 1;//back to first
                } else {
                    nCurWay = nCurWay - 2;
