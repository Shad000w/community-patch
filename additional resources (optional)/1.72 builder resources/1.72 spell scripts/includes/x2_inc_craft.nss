...
string GetMaterialComponentTag(int nPropID, string sCollumn="comp_tag")
{
    return Get2DAString("des_matcomp",sCollumn,nPropID);
}
...
object CICraftBrewPotion(object oCreator, int nSpellID )
{
    ...
    if (sMat != "")
    {
        ...
        {
            int nStack = GetItemStackSize(oMat);
    ...
    //1.71: new feature to specify custom potion to be created
...
int CICraftCheckBrewPotion(object oSpellTarget, object oCaster)
{
    ...
    // -------------------------------------------------------------------------
    if (nLevel > X2_CI_BREWPOTION_MAXLEVEL && nLevel > GetLocalInt(GetModule(),"X2_CI_BREWPOTION_MAXLEVEL") && nLevel > GetLocalInt(oCaster,"X2_CI_BREWPOTION_MAXLEVEL"))
    {
    ...
    if (GetIsObjectValid(oPotion))
    {
        ...
        if(nStack > 1)//1.71: support for stacking
        {
            SetItemStackSize(oSpellTarget,nStack-1);
        }
        else
        {
...
int CICraftCheckCraftWand(object oSpellTarget, object oCaster)
{
    ...
    // check if spell is below maxlevel for brew potions
    // -------------------------------------------------------------------------
    if (nLevel > X2_CI_CRAFTWAND_MAXLEVEL && nLevel > GetLocalInt(GetModule(),"X2_CI_CRAFTWAND_MAXLEVEL") && nLevel > GetLocalInt(oCaster,"X2_CI_CRAFTWAND_MAXLEVEL"))
    {
