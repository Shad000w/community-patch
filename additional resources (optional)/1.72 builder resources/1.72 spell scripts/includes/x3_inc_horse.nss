...
object HORSE_SupportGetHorseEffectCreator()
{ //1.72: all effects will be assigned to this object in order to recognize the effect comes from horse
object oCreator = GetObjectByTag("X3_EC_HORSE");
 if(!GetIsObjectValid(oCreator))
 {
 oCreator = CreateObject(OBJECT_TYPE_PLACEABLE,"plc_invisobj",GetStartingLocation(),FALSE,"X3_EC_HORSE");
 ...
 SetPlotFlag(oCreator,TRUE);
  ...
  {
  WriteTimestampedLogEntry("ERROR: Horse Effect Creator not found, some features might not work properly!");
  oCreator = GetModule();
  }
 }
return oCreator;
...
{ // PURPOSE: Change the movement speed based on that of the horse (wont work unless EffectMovementSpeedIncrease() bug is fixed)
    ...
    object oCreator = HORSE_SupportGetHorseEffectCreator();
    // EffectMovementSpeedIncrease() with negative numbers is the same as EffectMovementSpeedDecrease() with positive numbers
    AssignCommand(oCreator,ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectMovementSpeedIncrease(nSpeed)),oRider));
...
{ // PURPOSE: Remove speed increases based upon mounted
    ...
    {
        if(GetEffectType(eSearch) == EFFECT_TYPE_MOVEMENT_SPEED_INCREASE && GetTag(GetEffectCreator(eSearch)) == "X3_EC_HORSE")
        { // match
...
{ // PURPOSE: Check for feats and adjust penalties to archery while mounted
    ...
    { // Rider is mounted and has a ranged weapon - apply penalty
        int nNewPenalty = GetHasFeat(FEAT_MOUNTED_ARCHERY,oRider) ? 2 : 4;
        if(nNewPenalty != nMountedArcheryPenalty)
        {
            if(nMountedArcheryPenalty > 0)
            {
                // remove existing penalties before applying the new one
                eSearch = GetFirstEffect(oRider);
                while(GetIsEffectValid(eSearch))
                {
                   if(GetEffectType(eSearch) == EFFECT_TYPE_ATTACK_DECREASE && GetTag(GetEffectCreator(eSearch)) == "X3_EC_HORSE")
                   { // match
                       RemoveEffect(oRider,eSearch);
                   } // match
                   eSearch = GetNextEffect(oRider);
            ...
            object oCreator = HORSE_SupportGetHorseEffectCreator();
            AssignCommand(oCreator,ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectAttackDecrease(nNewPenalty)),oRider));
...
{ // PURPOSE: Applies decreases to skills while mounted
    ...
    object oCreator = HORSE_SupportGetHorseEffectCreator();
    AssignCommand(oCreator,ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectSkillDecrease(SKILL_DISABLE_TRAP,50)),oRider));
    AssignCommand(oCreator,ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectSkillDecrease(SKILL_OPEN_LOCK,50)),oRider));
    AssignCommand(oCreator,ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectSkillDecrease(SKILL_HIDE,50)),oRider));
    AssignCommand(oCreator,ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectSkillDecrease(SKILL_MOVE_SILENTLY,50)),oRider));
    AssignCommand(oCreator,ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectSkillDecrease(SKILL_PICK_POCKET,50)),oRider));
    AssignCommand(oCreator,ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectSkillDecrease(SKILL_SET_TRAP,50)),oRider));
    AssignCommand(oCreator,ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectSkillDecrease(SKILL_TUMBLE,50)),oRider));
...
{ // PURPOSE: To remove any negative effects caused by being mounted
    ...
    while(GetIsEffectValid(eSearch))
    {
        if(GetEffectType(eSearch) == EFFECT_TYPE_SKILL_DECREASE && GetTag(GetEffectCreator(eSearch)) == "X3_EC_HORSE")
        { // match
            RemoveEffect(oRider,eSearch);
        } // match
        eSearch = GetNextEffect(oRider);
...
{ // PURPOSE: Apply AC bonus
    ...
    if (nDiff<1) return;
    object oCreator = HORSE_SupportGetHorseEffectCreator();
    AssignCommand(oCreator,ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectACIncrease(nDiff,AC_NATURAL_BONUS)),oRider));
...
{ // PURPOSE: Remove AC Bonus
    ...
    while(GetIsEffectValid(eSearch))
    {
        if(GetEffectType(eSearch) == EFFECT_TYPE_AC_INCREASE && GetTag(GetEffectCreator(eSearch)) == "X3_EC_HORSE")
        { // match
            RemoveEffect(oRider,eSearch);
        } // match
        eSearch = GetNextEffect(oRider);
...
{ // PURPOSE: Apply HP bonus
    ...
    if (nHP<1) nHP=1;
    object oCreator = HORSE_SupportGetHorseEffectCreator();
    AssignCommand(oCreator,ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectTemporaryHitpoints(nHP)),oRider));
...
{ // PURPOSE: Remove HP Bonus
    ...
    while(GetIsEffectValid(eSearch))
    {
        if(GetEffectType(eSearch) == EFFECT_TYPE_TEMPORARY_HITPOINTS && GetTag(GetEffectCreator(eSearch)) == "X3_EC_HORSE")
        { // match
            RemoveEffect(oRider,eSearch);
        } // match
        eSearch = GetNextEffect(oRider);
...
{ // PURPOSE: Return whether oTarget is mounted
    switch(GetPhenoType(oTarget))//1.72: avoid using skin to prevent the skin creation in first place
    {
        case 3:
        case 5:
        case 6:
...
{ // PURPOSE: Add Horse Menu to the PC
    ...
    itemproperty iProp;
    if (GetIsPC(oPC) && !GetLocalInt(oSkin,"70_HorseMenuAdded"))//1.72: fix for stacking the horse menu bonus feat
    { // valid parameter
        iProp=ItemPropertyBonusFeat(IP_CONST_HORSE_MENU);
        AddItemProperty(DURATION_TYPE_PERMANENT,iProp,oSkin);
        SetLocalInt(oSkin,"70_HorseMenuAdded",1);
    } // valid parameter
} // HorseAddHorseMenu()
...
{ // PURPOSE: blocked path, death, entanglement, confusion, paralysis, stun, petrification, fear, turned, sleep, knockdown detection by Azbest
    ...
    else if(GetLocalInt(GetModule(),"X3_NO_MOUNT_COMMANDABLE") && !GetCommandable(oCreature))//1.72: added more accurate method but only in case horse system is set not to use SetCommandable
    {
        bDisabled=TRUE;
    }
    else
    {
        ...
        { // traverse effects
            if ((GetEffectType(eEffect)==EFFECT_TYPE_INVALIDEFFECT&&(GetEffectDurationType(eEffect)==DURATION_TYPE_TEMPORARY||GetEffectDurationType(eEffect)==DURATION_TYPE_PERMANENT)&&GetEffectSpellId(eEffect)!=SPELL_DIVINE_POWER)||//1.72: fix for divine power preventing mounting
...
{ // PURPOSE: See if not default appearance
    ...
    if(nApp >= 482 && nApp <= 869)//1.72: this will prevent forcing appearance change on players using custom appearance or players with non-standard racial type
    {
        int nRace=GetRacialType(oCreature);
        int nDefApp=StringToInt(Get2DAString("racialtypes","Appearance",nRace));
        if (nApp!=nDefApp) HorseChangeToDefault(oCreature);
    }
...
{ // PURPOSE: Force a preload of mount animations
    ...
    if (nTail==14) nTail=0;//1.72: null tail removed
