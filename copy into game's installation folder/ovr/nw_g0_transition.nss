...
void TeleportPlayer(object oTarget)
{
    if(GetIsObjectValid(oTarget))
    {
        ...
        if(GetArea(oTarget) != GetArea(OBJECT_SELF) || !GetModuleSwitchValue("72_JUMP_ASSOCIATES_WITH_PLAYER")) return;
        object oAssoc = GetFirstFactionMember(OBJECT_SELF,FALSE);
        while(oAssoc != OBJECT_INVALID)
        {
            if(!GetIsPC(oAssoc) && GetMaster(oAssoc) == OBJECT_SELF)
            {
                AssignCommand(oAssoc,ClearAllActions());
                AssignCommand(oAssoc,JumpToObject(oTarget));
            }
            oAssoc = GetNextFactionMember(OBJECT_SELF,FALSE);
        }
    }
}
...
void main()
{
    ...
    { // delayed jump
        ...
        DelayCommand(fDelay+0.1*fX3_MOUNT_MULTIPLE,AssignCommand(oClicker,TeleportPlayer(oTarget)));
    } // delayed jump
    else if(GetIsPC(oClicker) && !GetIsPossessedFamiliar(oClicker))
    {
        AssignCommand(oClicker,TeleportPlayer(oTarget));
    }
    else
    { // quick jump
