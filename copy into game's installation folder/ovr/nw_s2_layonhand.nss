...
void main()
{
    ...
    //--------------------------------------------------------------------------
    if(spellsIsRacialType(oTarget, RACIAL_TYPE_UNDEAD) || GetLevelByClass(CLASS_TYPE_UNDEAD,oTarget)>0)
    {
        ...
        //----------------------------------------------------------------------
        //int nResist = MyResistSpell(OBJECT_SELF,oTarget);//1.71: removed, was nonfunctional anyway
        //if (nResist == 0 )
        {
            if(nTouch > 0)
            {
                ...
                //SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, SPELLABILITY_LAY_ON_HANDS));//its signaled already
