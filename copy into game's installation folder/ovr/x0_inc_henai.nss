...
void HenchmenCombatRound(object oIntruder)
{
    ...
    //------------------------------------------------------------------------------
    if(ResManGetAliasFor("70_ai_henchman",RESTYPE_NSS) != "")
    {
...
/*    int iSurrendered = GetLocalInt(OBJECT_SELF,"Generic_Surrender");
    ...
    case ASSOCIATE_COMMAND_DISARMTRAP: // Disarm trap
        ...
        if(!GetIsObjectValid(oTarget) || !GetIsTrapped(oTarget) || !GetTrapDetectedBy(oTarget, OBJECT_SELF) || GetDistanceToObject(oTarget) > 15.0 || !bkGetIsInLineOfSight(oTarget))
        {
    ...
    case ASSOCIATE_COMMAND_HEALMASTER:
        ...
        if(GetIsDead(oMaster))//1.71: taught henchman to resurrect his master
        {
            ...
            {
                if(bkTalentFilter(TalentSpell(SPELL_RESURRECTION),oMaster))
                    DelayCommand(2.0, VoiceCanDo());
                break;
            }
            else if(GetHasSpell(SPELL_RAISE_DEAD))
            {
                if(bkTalentFilter(TalentSpell(SPELL_RAISE_DEAD),oMaster))
                    DelayCommand(2.0, VoiceCanDo());
                break;
            }
            int nRnd;
...
int bkCombatAttemptHeal()
{
   ...
   // if (bkGetBehavior(BK_HEALINMELEE) == FALSE)//1.72: unused, was always false
   //     iHealMelee = FALSE;
...
//      that they are in NEVER FIGHT mode
    if (bkGetBehavior(BK_NEVERFIGHT))
    {
