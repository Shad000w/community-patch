...
itemproperty IPGetItemPropertyByID(int nPropID, int nParam1 = 0, int nParam2 = 0, int nParam3 = 0, int nParam4 = 0)
{
   ...
   else if (nPropID == ITEM_PROPERTY_DECREASED_DAMAGE) //1.72: added missing itemproperty
   {
   ...
   else if (nPropID == ITEM_PROPERTY_SAVING_THROW_BONUS) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyBonusSavingThrow(nParam1, nParam2);
   }
   else if (nPropID == ITEM_PROPERTY_SAVING_THROW_BONUS_SPECIFIC) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyBonusSavingThrowVsX(nParam1, nParam2);
   }
   else if (nPropID == ITEM_PROPERTY_SPELL_RESISTANCE) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyBonusSpellResistance(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_IMMUNITY_DAMAGE_TYPE) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyDamageImmunity(nParam1,nParam2);
   }
   else if (nPropID == ITEM_PROPERTY_IMMUNITY_MISCELLANEOUS) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyImmunityMisc(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_IMMUNITY_SPECIFIC_SPELL) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertySpellImmunitySpecific(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_IMMUNITY_SPELL_SCHOOL) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertySpellImmunitySchool(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_IMMUNITY_SPELLS_BY_LEVEL) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyImmunityToSpellLevel(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_FREEDOM_OF_MOVEMENT) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyFreeAction();
   }
   else if (nPropID == ITEM_PROPERTY_HEALERS_KIT) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyHealersKit(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_HOLY_AVENGER) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyHolyAvenger();
   }
   else if (nPropID == ITEM_PROPERTY_IMPROVED_EVASION) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyImprovedEvasion();
   }
   else if (nPropID == ITEM_PROPERTY_USE_LIMITATION_ALIGNMENT_GROUP) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyLimitUseByAlign(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_USE_LIMITATION_CLASS) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyLimitUseByClass(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_USE_LIMITATION_RACIAL_TYPE) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyLimitUseByRace(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_USE_LIMITATION_SPECIFIC_ALIGNMENT) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyLimitUseBySAlign(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_MIGHTY) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyMaxRangeStrengthMod(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_MONSTER_DAMAGE) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyMonsterDamage(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_ON_MONSTER_HIT) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyOnMonsterHitProperties(nParam1,nParam2);
   }
   else if (nPropID == ITEM_PROPERTY_REGENERATION) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyRegeneration(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_SKILL_BONUS) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertySkillBonus(nParam1,nParam2);
   }
   else if (nPropID == ITEM_PROPERTY_SPECIAL_WALK) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertySpecialWalk(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_THIEVES_TOOLS) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyThievesTools(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_TURN_RESISTANCE) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyTurnResistance(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_REGENERATION_VAMPIRIC) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyVampiricRegeneration(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_VISUALEFFECT) //1.72: added missing itemproperty
   {
        ipRet = ItemPropertyVisualEffect(nParam1);
   }
   else if (nPropID == ITEM_PROPERTY_WEIGHT_INCREASE) //1.72: added missing itemproperty
   {
...
// 1.70: bludgeoning+piercing weapons like morningstars are now considered to be bludgeoning as well.
// ----------------------------------------------------------------------------
int IPGetIsBludgeoningWeapon(object oItem)
{
    int nBaseItem = GetBaseItemType(oItem);
...
object IPGetModifiedArmor(object oArmor, int nPart, int nMode, int bDestroyOldOnSuccess)
{
    ...
    SetItemCursedFlag(oNew,GetItemCursedFlag(oArmor));//1.72: restoration of the undroppable flag
...
object IPGetTargetedOrEquippedMeleeWeapon(object oTarget = OBJECT_INVALID)
{
    ...
    {
        oTarget = GetLocalObject(OBJECT_SELF,"SPELL_TARGET_OVERRIDE");//1.72: accounting for target override
        if(!GetIsObjectValid(oTarget)) oTarget = GetSpellTargetObject();
    }
    if(GetIsObjectValid(oTarget) && GetObjectType(oTarget) == OBJECT_TYPE_ITEM && !GetLocalInt(oTarget,"72_DISABLE_ENCHANTMENT_SPELLS"))
    {
        if(GetBaseItemType(oTarget) == BASE_ITEM_GLOVES)
        {
            return GetModuleSwitchValue("71_ALLOW_BOOST_GLOVES") ? oTarget : OBJECT_INVALID;
        }
        else if(IPGetIsProjectile(oTarget))
        {
            return GetModuleSwitchValue("72_ALLOW_BOOST_AMMO") ? oTarget : OBJECT_INVALID;
        }
        else if(GetWeaponRanged(oTarget))
        {
            if(IPGetIsThrownWeapon(oTarget))
            {
                return GetModuleSwitchValue("72_ALLOW_BOOST_THROWING") ? oTarget : OBJECT_INVALID;
            }
            return GetModuleSwitchValue("72_ALLOW_BOOST_AMMO") ? oTarget : OBJECT_INVALID;
        }
        else if(IPGetIsMeleeWeapon(oTarget))
        {
            return oTarget;
        }
    ...
    if(GetIsObjectValid(oWeapon1) && !GetLocalInt(oWeapon1,"72_DISABLE_ENCHANTMENT_SPELLS"))
    {
        if(GetWeaponRanged(oWeapon1))
        {
            if(IPGetIsThrownWeapon(oWeapon1))
            {
                if(GetModuleSwitchValue("72_ALLOW_BOOST_THROWING"))
                {
                    return oWeapon1;
                }
            }
            else if(GetModuleSwitchValue("72_ALLOW_BOOST_AMMO"))
            {
                return oWeapon1;
            }
        }
        else if(IPGetIsMeleeWeapon(oWeapon1))
        {
            return oWeapon1;
        }
    }

    //this will happen if player is holding non weapon, unsupported custom weapon or weapon disallowed to be enchanted
    oWeapon1 = GetItemInSlot(INVENTORY_SLOT_LEFTHAND,oTarget);
    if(GetIsObjectValid(oWeapon1) && !GetLocalInt(oWeapon1,"72_DISABLE_ENCHANTMENT_SPELLS") && IPGetIsMeleeWeapon(oWeapon1))
    {
        return oWeapon1;
    }

    //gloves - only if allowed by module switch
    oWeapon1 = GetItemInSlot(INVENTORY_SLOT_ARMS,oTarget);
    if(GetBaseItemType(oWeapon1) == BASE_ITEM_GLOVES && !GetLocalInt(oWeapon1,"72_DISABLE_ENCHANTMENT_SPELLS") && GetModuleSwitchValue("71_ALLOW_BOOST_GLOVES"))
    {
        return oWeapon1;
    }
    ...
    oWeapon1 = GetItemInSlot(INVENTORY_SLOT_CWEAPON_L,oTarget);//1.70: left creature weapon was ommited
    if(GetIsObjectValid(oWeapon1) && !GetLocalInt(oWeapon1,"72_DISABLE_ENCHANTMENT_SPELLS"))
    {
        return oWeapon1;
    }
...
object IPGetTargetedOrEquippedArmor(int bAllowShields = FALSE, object oTarget = OBJECT_INVALID)
{
    ...
    if(GetIsObjectValid(oArmor1) && GetBaseItemType(oArmor1) == BASE_ITEM_ARMOR && !GetLocalInt(oArmor1,"72_DISABLE_ENCHANTMENT_SPELLS"))
    {
        return oArmor1;
    }
    if(bAllowShields)
    {
        ...
        if(GetIsObjectValid(oArmor1) && !GetLocalInt(oArmor1,"72_DISABLE_ENCHANTMENT_SPELLS"))
        {
...
object IPGetModifiedWeapon(object oWeapon, int nPart, int nMode, int bDestroyOldOnSuccess)
{
    ...
    SetItemCursedFlag(oNew,GetItemCursedFlag(oWeapon));//1.72: restoration of the undroppable flag
    if (oNew != OBJECT_INVALID)
    {
        if( bDestroyOldOnSuccess )
        {
...
object IPCreateAndModifyArmorRobe(object oArmor, int nRobeType)
{
    ...
    SetItemCursedFlag(oRet,GetItemCursedFlag(oArmor));//1.72: restoration of the undroppable flag
